...

{
    ...
    public function edituserdetails(Request $request){
        ...
        $usemail = User::where([["email", $request->email],['id','!=',$userid]])->first();
        $usphone = User::where([["phone", $request->phone],['id','!=',$userid]])->first();
        if(!empty($usemail)){
            return response()->json(['status' => false, 'message' => "This email is registerd for another account"], 200);
        }

        if(!empty($usphone)){
            return response()->json(['status' => false, 'message' => "This Number is registerd for another account"], 200);
        }
       ...
       if(!empty($request->profile)){
        $img = 'data:image/jpeg;base64,'.$request->profile;
        $path = 'user-profile/';
...

    {
       ...
       // dd($user_id);
        ...
        $billing= Address::where([['user_id','=',$user_id],['address_type','shipping']])->get();
        $shipping= Address::where([['user_id','=',$user_id],['address_type','shipping']])->get();

        $newarray['billing'] = $billing;
        $newarray['shipping'] = $shipping;

        if(count($newarray) > 0){

             return response()->json([ 'status'=> true , 'message' => "success", 'address' => $newarray], 200);
       ...
       if(Auth::attempt(['email' => request('email'), 'password' => request('password')])){
             ...
             if($user->is_mobile_verified == 1){
                ...
                if(!empty($req->fcm_token)){
                    $existtoken = UserDeviceToken::where('device_id',$req->fcm_token)->first();
                    if(empty($existtoken)){
                       UserDeviceToken::create([
                       'user_id' => $user->id,
                       'device_id' => $req->fcm_token
                       ]); 
                    }
                    //update user data in wishlish and cart
                    $this->addDataToCartAndWishlist($req->fcm_token,$user->id);
                }
                ...
                return response()->json(['status' => true,'message' => "Your account logged in successfully",'token'=>$token, 'user' => $user], 200);
                    ...
                    return response()->json(['status' => false,'message' => 'User mobile is not verfied'], 200);
             }
    ...
    public function addDataToCartAndWishlist($deviceId,$userId){
        $getguestData = GuestUserData::where('device_id',$deviceId)->get();
        $variations = null;
        foreach($getguestData as $key => $val){
              $Product = Product::where('id',$val->product_id)->first();
            if($val->type == 'wishlist'){
                $exist = Wishlist::where([['user_id',$userId],['product_id',$val->product_id]])->first();
              if(empty($exist)){
                 Wishlist::create([
                                'user_id' => $userId,
                                'product_id' => $val->product_id
                        ]);
                  GuestUserData::where('id',$val->id)->delete();
              }
             
            }
            elseif($val->type == 'cart'){
                 $exist = Cart::where([['user_id',$userId],['product_id',$val->product_id]])->first();
                  if(empty($exist)){
                     Cart::create([
                        'user_id'             => $userId,
                        'product_id'          => $val->product_id,
    ...
    public function register(Request $request)
    {
        ...
        $usemail = User::where("email", $request->email)->first();
        $usphone = User::where("phone", $request->phone)->first();
        $userexistwithdetails = User::where([["phone", $request->phone],["email", $request->email]])->first();
        if(!empty($userexistwithdetails)){
            //Send otp
            //$this->sendOTP($userexistwithdetails->phone);
          return response()->json(['status' => true, 'message' => "please verify user mobile"], 200);
        }
        if(!empty($usemail)){
        ...
        if(!empty($usphone)){
            return response()->json(['status' => false, 'message' => "Your account with this number already registerd."], 200);
        }
        ...
        if(!empty($request->fcm_token)){
            $existtoken = UserDeviceToken::where('device_id',$request->fcm_token)->first();
            if(!empty($existtoken)){
               UserDeviceToken::create([
               'user_id' => $user->id,
       ...
       // $this->sendOTP($mobile,$)
        ...
        return response()->json(['status' => true, 'message' => "Your account registerd successfully."], 200);
    ...
    public function resendOTP(Request $request){
        try {
            ...
            return response()->json([
                'status' => false, 'message' => "Enterd mobile number is invalid"], 200);
        }
     ...
     public function verifyOTP(Request $req)
    {
        ...
        try {
            ...
            if ($verification_check->status == 'approved') {
                ...
                return response()->json([
                    'status' => true,
                    'message' => " Your mobile number verified successfully"], 200);
            } else {

                return response()->json([
                    'status' => false,
                    'message' => " Invalid OTP, Please enter valid OTP or Resend OTP"], 200);
            }
        ...
        if(!empty($usphone)){
            ...
            $vendordetail = array('store_url'=> $request->store_url,'store_name'=> $request->store_name,'store_location'=> $request->store_location);
               ...
               foreach ($vendordetail as $key => $value) {
                ...
                $mail_data = Mails::where('msg_category', 'vendor-signup')->first();
        ...
        if ($validator->fails()) { 
            ...
            if(!empty($data)){
                if(Hash::check($request->old_password, $data->password)){
                    ...
                    return response()->json(['status'=>false,'message'=>'Your Old Password does not match'] );   
                }
