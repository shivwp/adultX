...
class OrderApiController extends Controller
{
    ...
    public function dateAvailability(Request $request){
        ...
        elseif($product->in_stock < $request->qty){
            return response()->json(['status' => false, 'message' => "qty not found"], 200);
        }
        ...
        $cusdates = array();
         ...
         $current = Carbon::now()->createFromFormat('F Y', $request->month.' '.$request->year);
        ...
        for ($i = 1; $i < $current->daysInMonth + 1; ++$i) {
            ...
            // Availablity Check Bookings       
            $booked = OrderedProducts::where([['product_id', '=', $request->product_id],['product_type', '=', 'booking']])->whereDate('start_date', '<=', $c_data)
                                ->whereDate('end_date', '>=', $c_data)->first();
            // Availablity Check from product Availablity   
            $avaliableCheck = ProductAvailability::where('product_id', '=', $request->product_id)
                                ->whereDate('from_date', '<=', $c_data)
                                ->whereDate('to_date', '>=', $c_data)
                                ->first();
            $product_qty = $product->in_stock;
             if (!empty($avaliableCheck)) {
                 $c_data = date('Y-m-d', strtotime($c_data));
                  ...
                  if (($c_data >= $startDate) && ($c_data <= $endDate)) {
                    $product_qty = $avaliableCheck->qty;
                  } else {
                    $product_qty = $product->in_stock;
                  }

             }
              $avaliableProducts = $product_qty - (!empty($booked->quantity) ? $booked->quantity : 0);
                if ($avaliableProducts >= $request->qty) {
                  $avaliable = true;
                }
             ...
             $avaliableProduct = $product_qty - (!empty($booked->quantity) ? $booked->quantity : 0);
            $dates[] = ['month' => $monthName,'date' => $c_data, 'month_no' => $monthNo,'is_available' => $avaliable];   
        ...
        $orders = Order::where('user_id','=',$userid)->orderBy('orders.created_at','DESC')->where('parent_id','=',0)->with('orderItem')->get();
           if(count($orders) >0 ){
             ...
             foreach($orders as $key => $val){
                $orders[$key]['created_date'] = date("d-m-Y", strtotime($val->created_at));
                ...
                $meta = OrderMeta::where('order_id',$val->id)->pluck('meta_value','meta_key');
                unset($meta['billing_address']);
                unset($meta['shipping_address']);
                $meta['shipping'] = json_decode($meta['shipping']);
                $orders[$key]['order_meta'] = $meta;
                foreach($val->orderItem as $orderitemkey => $ordereditem){
                    $orderproductmeta = OrderProductMeta::where('product_id',$ordereditem->product_id)->pluck('meta_value','meta_key');
                    $val->orderItem[$orderitemkey]['featured_image'] = !empty($orderproductmeta['product_image']) ?  url('products/feature/'. $orderproductmeta['product_image']) : '';
                }
